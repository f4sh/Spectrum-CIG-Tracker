document.addEventListener('DOMContentLoaded', function () {
    ...
    chrome.storage.local.get(['selectedUsers', 'interval', 'tracking', 'trackingStartTime', 'notificationsShown', 'notificationsClicked', 'shownMessageIds'], (result) => {
    ...
    function saveSettings() {
        ...
        chrome.storage.local.set({ selectedUsers, interval }, () => {
            console.log("Settings saved:", { selectedUsers, interval });

            if (toggleTrackingButton.textContent === 'Stop Tracking') {
                chrome.runtime.sendMessage({ action: 'changeInterval', interval: Number(interval) });
    ...
    toggleTrackingButton.addEventListener('click', () => {
        ...
        if (toggleTrackingButton.textContent === 'Start Tracking') {
            ...
            chrome.storage.local.set({ selectedUsers, interval, tracking: true, trackingStartTime: currentStartTime }, () => {
                startUpdatingRunningTime(currentStartTime);
            });
            toggleTrackingButton.textContent = 'Stop Tracking';
            chrome.runtime.sendMessage({ action: 'startTracking', users: selectedUsers, interval: Number(interval) });
            statsDiv.style.display = 'block';
        } else {
            chrome.storage.local.set({ tracking: false, trackingStartTime: null }, () => {
    ...
    function updateStats() {
        ...
        chrome.storage.local.set({ notificationsShown, notificationsClicked, shownMessageIds });
    }
...
function saveMotdSetting() {
    ...
    chrome.storage.local.set({ trackMotd }, () => {
